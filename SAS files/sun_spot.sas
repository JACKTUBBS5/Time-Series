...
data c; set a ; new_count = count + 2;
...
proc gplot data=c; plot count*N=1;
run;

proc arima data=c;
/*  Look at the input series oil  */
    i var=scount(1 11)noprint ;run;
   e  p=( 1 3 5 6) q=(2  8 10 11 ) noint noprint;run;
    f    lead=24 back=12  id=N out=out1;
    f    lead=12 id=N out=out2;
run;
   proc gplot data=out1; where n > 250;
   plot scount*n=1 forecast*n=2/overlay;
   run;
proc gplot data=out2; where n > 250;
   plot scount*n=1 forecast*n=2 l95*n=3 u95*n=3/overlay;
   run;
