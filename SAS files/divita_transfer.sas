...
symbol1 i=none  v=star; 
   ...
   proc autoreg data=c;
   model consumer = department/dw=4 dwprob nlag=1 method=ml;
...
proc gplot data=p; where n>150;
   ...
   run;
title2 'State Space Model for Starts';

  proc statespace data=c out=out lead=10 cancorr; 
      var department(1,12) consumer(1,12); 
      id N; 
   run;
   proc print data=out;where n > 100;run;
proc gplot data=out; 
      plot   for2*N=2   consumer*N=1 / overlay;
where n > 100; 
   run;
   data new; merge b out; by n; keep n forecast consumer department for1 for2;run;
   proc gplot data=new; 
      plot   for2*N=2 forecast*n=3  consumer*N=1 / overlay;
* where n > 100; 
   run;
   proc print data=new; where n>100;run;
   quit;